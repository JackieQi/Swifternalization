...
class InequalityExpressionParserTests: XCTestCase {
    ...
    // MARK: - Int
    func testIE1() {
        let m = createMatcher(pattern: "ie:x=3")
    ...
    func testIE6() {
        let m = createMatcher(pattern: "ie:x=11")
        XCTAssertEqual(m.sign, InequalitySign.Equal, "")
        XCTAssertEqual(m.value, 11, "")
    }
    
    func testIE7() {
        let m = createMatcher(pattern: "ie:x=-5")
        XCTAssertEqual(m.sign, InequalitySign.Equal, "")
    ...
    // MARK: - Float
    func testIE8() {
        let m = createMatcher(pattern: "ie:x=3.0")
        XCTAssertEqual(m.sign, InequalitySign.Equal, "")
        XCTAssertEqual(m.value, 3.0, "")
    }
    
    func testIE9() {
        let m = createMatcher(pattern: "ie:x<3.6")
        XCTAssertEqual(m.sign, InequalitySign.LessThan, "")
        XCTAssertEqual(m.value, 3.6, "")
    }
    
    func testIE10() {
        let m = createMatcher(pattern: "ie:x>-44.45")
        XCTAssertEqual(m.sign, InequalitySign.GreaterThan, "")
